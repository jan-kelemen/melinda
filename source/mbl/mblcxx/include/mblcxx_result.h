...
public: // Construction
    ...
    result(std::in_place_index_t<I> index, Args&&... value) noexcept(
        std::is_nothrow_constructible_v<std::variant_alternative_t<I, either_t>,
            Args...>)
        requires(
...
template<typename T, typename E>
template<typename... Args>
result<T, E> res<T, E>::ok(Args&&... args) noexcept(
    noexcept(result<T, E>(std::in_place_index<0>, std::forward<Args>(args)...)))
{
...
template<typename T, typename E>
template<typename... Args>
result<T, E> res<T, E>::error(Args&&... args) noexcept(
    noexcept(result<T, E>(std::in_place_index<1>, std::forward<Args>(args)...)))
{

...
enum class [[nodiscard]] trace_level : uint8_t {
    ...
    error = 4,
    fatal = 5,
    always = std::numeric_limits<std::underlying_type_t<trace_level>>::max(),
};
...
public: // Interface
    [[nodiscard]] bool should_trace_message(trace_level level) const noexcept;
...
[[nodiscard]] trace_handle create_trace_handle(trace_options const& options);
...
inline void trace(trace_handle const& handle, std::string_view message) noexcept
{

...
private: // Helpers
    ...
    [[nodiscard]] int open_file(std::filesystem::path const& full_path) const;
    ...
    std::string_view message) const noexcept
{
    ...
    std::vector<std::filesystem::path> matching_entries = find_matching_files();
    ...
    std::nth_element(std::begin(matching_entries),
        ...
        [](std::filesystem::path const& lhs, std::filesystem::path const& rhs)
        {
...
int melinda::mbltrc::trace_handle::impl::open_latest_file() const
{
    ...
    std::filesystem::path const& p = *std::max_element(
        std::cbegin(matching_entries),
        std::cend(matching_entries),
        [](std::filesystem::path const& lhs, std::filesystem::path const& rhs)
        {
            return std::filesystem::last_write_time(lhs) <

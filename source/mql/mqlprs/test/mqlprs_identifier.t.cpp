...
namespace
{
    ...
    { return mqlprs::parse<mqlast::identifier>(query); };

    template<typename T>
    void check_for_identifier_type(std::string_view expected_body,
        mqlast::identifier const& id,
...
TEST_CASE("<identifier> recognizes correct variant of identifier")
{
    ...
    check_for_identifier_type<mqlast::regular_identifier>("abc",
        parse(regular_identifier).value());

    auto const delimited_identifier = R"("abc")"sv;
    check_for_identifier_type<mqlast::delimited_identifier>("abc",
        parse(delimited_identifier).value());

    auto const unicode_delimited_identifier = R"(U&"abc")"sv;
    check_for_identifier_type<mqlast::unicode_delimited_identifier>("abc",
